...
class MetaRLAlgorithm(metaclass=abc.ABCMeta):
    ...
    def train(self):
        ...
        # assign fixed latent vectors to the training tasks
        # not used when testing
        n_train_tasks = len(self.train_tasks)
        self.z_train = ptu.normal(mean=0., std=1., size=(n_train_tasks, self.agent.latent_dim))
    ...
    def evaluate(self, epoch):
        ...
        for idx in indices:
            ...
            for _ in range(self.num_steps_per_eval // self.max_path_length):
                ...
                # for i in range(len(p[0]['observations'])):
                #     obs = p[0]['observations'][i]#ptu.from_numpy(obs[None])
                #     action = p[0]['actions'][i]
                #     self.env.render(obs, action)
                # self.env.close()
