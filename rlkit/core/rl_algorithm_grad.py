...
class MetaRLAlgorithm(metaclass=abc.ABCMeta):
        ...
        for it_ in gt.timed_for(
            ...
            for i in range(self.num_tasks_sample):
                ...
                if self.num_steps_prior > 0:
                    self.collect_data(self.num_steps_prior, 1,
                                      np.inf, decoder=decoder)

                # collect some trajectories with z ~ posterior
                if self.num_steps_posterior > 0:
                    self.collect_data(self.num_steps_posterior,
                                      1, self.update_post_train, decoder=decoder)

                # even if encoder is trained only on samples from the prior, the policy needs to learn to handle z ~ posterior
                if self.num_extra_rl_steps_posterior > 0:
                    self.collect_data(self.num_extra_rl_steps_posterior, 1,
                                      self.update_post_train, add_to_enc_buffer=False, decoder=decoder)

            # Sample train tasks and compute gradient updates on parameters.
            if it_ == 0:
                num_pretrain_step = 0  # self.num_train_steps_per_itr
            else:
                num_pretrain_step = 0  # self.num_train_steps_per_itr

            """
            for pretrain_step in range(num_pretrain_step):
                indices = np.random.choice(self.train_tasks, self.meta_batch)
                decoder_loss=self._do_pretraining(indices)
                if pretrain_step % 100 == 0:
                    print("Iter : ", pretrain_step, " Regression loss : ", decoder_loss)
            """

            for train_step in range(self.num_train_steps_per_itr):

                indices = np.random.choice(self.train_tasks, self.meta_batch)
                self._do_training(indices, iter_pre=train_step)
    ...
    def collect_data(self, num_samples, resample_z_rate, update_posterior_rate, add_to_enc_buffer=True, init=False, decoder=None):
        '''
        ...
        while num_transitions < num_samples:
            paths, n_samples = self.sampler.obtain_samples(max_samples=num_samples - num_transitions,
                                                           ...
                                                           accum_context=False,
                                                           resample=resample_z_rate, init=init, decoder=decoder)
    ...
    def collect_paths(self, idx, epoch, run):
        ...
        while num_transitions < self.num_steps_per_eval:
            path, num = self.sampler.obtain_samples(
                deterministic=self.eval_deterministic, max_samples=self.num_steps_per_eval - num_transitions, max_trajs=1, accum_context=True)
            paths += path
            num_transitions += num
            num_trajs += 1
        ...
        if self.dump_eval_paths:
            logger.save_extra_data(
                paths, path='eval_trajectories/task{}-epoch{}-run{}'.format(idx, epoch, run))
    ...
    def evaluate(self, epoch):
        ...
        for idx in indices:
            ...
            for _ in range(self.num_steps_per_eval // self.max_path_length):
                ...
                # one generated by the encoder, and the other generated by gradient descent
                self.agent.infer_posterior(context)
        ...
        avg_train_online_return = np.mean(
            np.stack(train_online_returns), axis=0)
