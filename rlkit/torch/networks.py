...
class Decoder(PyTorchModule):
    def __init__(self,
        ...
        self.fc_layers.append(nn.Linear(curr_input_dim, 256))
        self.fc_layers.append(nn.Linear(256, 256))
        self.fc_layers.append(nn.Linear(256, 256))

        # reward_dim = 1
        self.fc_output=nn.Linear(256, state_dim)


    def forward(self, state, actions, latent_state):

        h = torch.cat((state, actions, latent_state), dim=-1)
